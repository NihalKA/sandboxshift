...
def _parse_env_var(env_str: str) -> tuple[str, str]:
    """Parse a KEY=VAL string into (key, value). Raises ValueError on bad format."""
    if "=" not in env_str:
        raise ValueError(
            f"Invalid --env format {env_str!r}: expected KEY=VALUE (e.g. NODE_ENV=production)"
        )
    key, _, value = env_str.partition("=")
    key = key.strip()
    if not key:
        raise ValueError(f"Invalid --env format {env_str!r}: key must not be empty")
    return key, value
...
async def _run_async(args: argparse.Namespace, workspace: Path) -> RunResult:
    ...
    # env_vars: parse CLI --env KEY=VAL flags; CLI wins over YAML env: section.
    if args.env_vars:
        effective_env_vars: dict[str, str] = {}
        for env_str in args.env_vars:
            try:
                k, v = _parse_env_var(env_str)
                effective_env_vars[k] = v
            except ValueError as e:
                print(f"Error: {e}", file=sys.stderr)
                sys.exit(1)
    else:
        effective_env_vars = dict(yaml_cfg.get("env_vars", {}))
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    run_parser.add_argument(
        ...
        help=(
            ...
            "(e.g. --env COMPONENTS_UI_NPM_TOKEN=ghp_xxx --env NODE_ENV=production). "
            "Repeat for multiple. CLI replaces YAML env: section entirely when set. "
