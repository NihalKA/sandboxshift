...
#   5. Runs terraform init + terraform apply (all variables auto-derived)
...
# ───────────────────────────────────────────────────────────────────────────────
# Generate unique names using account ID + random 6-char hash
# ───────────────────────────────────────────────────────────────────────────────

# Persistent hash: stored in ~/.sandboxshift/setup.env so re-running
# the script uses the same bucket names (idempotent).
...
if [[ -f "${SETUP_ENV}" ]]; then
  ...
  WORKSPACE_BUCKET="sandboxshift-ws-${ACCOUNT_ID}-${RAND_HASH}"
...
info "Workspace bucket: ${WORKSPACE_BUCKET}"
...
# ───────────────────────────────────────────────────────────────────────────────
# Step 5: terraform apply (all vars auto-derived, no prompts)
# ───────────────────────────────────────────────────────────────────────────────

info "Running terraform apply..."
terraform apply -auto-approve \
  -var="aws_region=${REGION}" \
  -var="workspace_bucket_name=${WORKSPACE_BUCKET}" \
